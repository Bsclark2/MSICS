...
def GDL(inp,D,N,T):
    delr,taud=inp
    return 0.3535*1.0/N*np.exp(-1.0*(delr**2)/(w0**2+4*D*(np.abs(taud))))/(1.0+4*D*(np.abs(taud))/w0**2)/np.sqrt(1.0+4*D*(np.abs(taud))/ecc**2/w0**2)*(1+T*(np.exp(-taud/tauT)-1.0))*(1.0-T)+1.0
