...
algorithm the pure diffusion function is used. We have also transformed xi,psi into
...
are: GDL((delr,taud),Dif. Coeff., Mean Particle Num.)
"""
def GDL(inp,D,N):
    delr,taud=inp
    return 0.3535*1.0/N*np.exp(-1.0*(delr**2)/(w0**2+4*D*(np.abs(taud))))/(1.0+4*D*(np.abs(taud))/w0**2)/np.sqrt(1.0+4*D*(np.abs(taud))/ecc**2/w0**2)+1.0
def flat(lis):
    flatList = []
...
for k in range(1,500):
    filename='E:\Ben\MSICS Frames\RHO6GREVIEW\RHO6G50G20us_'+f'{k:03}'+'.txt'
    intdata = pd.read_csv(filename,sep="\t",header=None,skiprows=3,engine='python',encoding='unicode_escape')
...
for i in range(0,Sfactor+1):
    if i%2==1:
        Sdata=Gmstack[immobilefactor2:ACFlen+immobilefactor2,rowstart:rowstart+ROIheight-i,collumnstart:collumnstart+ROIlength]
        NSdata=Gmstack[immobilefactor2:ACFlen+immobilefactor2,rowstart+i:rowstart+ROIheight,collumnstart:collumnstart+ROIlength]
...
for j in range(1,Sfactor+1):
    Sdata=Gmstack[immobilefactor2:ACFlen+immobilefactor2,rowstart:rowstart+ROIheight,collumnstart:collumnstart+ROIlength-j]
    NSdata=Gmstack[immobilefactor2:ACFlen+immobilefactor2,rowstart:rowstart+ROIheight,collumnstart+j:collumnstart+ROIlength]
    Sdata[:,::2, :] = Sdata[:,::2, ::-1]
    NSdata[:,::2, :] = NSdata[:,::2, ::-1]
...
plt.figure(dpi=500)
plt.scatter(timearray[1381:],disarray[1381:],c=corarray[1381:],cmap='inferno')
plt.scatter(timearray[:1380],disarray[:1380],c=corarray[:1380],cmap='inferno')
...
Na=6.022e23
Veff=1.62e-15
popt,pcov=curve_fit(GDL, (disarray,timearray),corarray,p0=[32.0e-12, 2.19193849],method='lm')
Con=1/popt[1]/Na/Veff
print('total fit para '+str(popt[0]/10.0**(-12)))
print('total fit para '+str(popt[1]))
print('total fit cov '+str(pcov))
print(Con)
popt,pcov=curve_fit(GDL, (flat(displacement[0:15]),flat(Timelag[0:15])),flat(CorrelationV[0:15]),p0=[32.0e-12, 2.19193849],method='lm')
Con=1/popt[1]/Na/Veff
print('total fit para '+str(popt[0]/10.0**(-12)))
print('total fit para '+str(popt[1]))
print('total fit cov '+str(pcov))
print(Con)
popt,pcov=curve_fit(GDL, (flat(displacement[16:]),flat(Timelag[16:])),flat(CorrelationV[16:]),p0=[32.0e-12, 2.19193849],method='lm')
Con=1/popt[1]/Na/Veff
print('total fit para '+str(popt[0]/10.0**(-12)))
print('total fit para '+str(popt[1]))
print('total fit cov '+str(pcov))
...
plt.figure(dpi=500)
plt.scatter(timearray[1381:],corarray[1381:],c=disarray[1381:],cmap='inferno_r')
plt.plot(np.array(timearray[1381:]),GDL((np.array(disarray[1381:]),np.array(timearray[1381:])),popt[0],popt[1]),c='b')
plt.scatter(timearray[:1380],corarray[:1380],c=disarray[:1380],cmap='inferno_r')
plt.plot(np.array(timearray[:1380]),GDL((np.array(disarray[:1380]),np.array(timearray[:1380])),popt[0],popt[1]),c='b')
plt.xlabel("Timelag $\\tau$ (s)")
plt.ylabel("Correlation G($\\tau$,$\Delta r$)")
plt.xscale('log')
# plt.colorbar()
...
#%%
plt.figure(dpi=500)
plt.scatter(np.array(timearray),np.array(disarray),c=GDL((np.array(disarray),np.array(timearray)),popt[0],popt[1]),cmap='inferno')
plt.xlabel("Timelag $\\tau$ (s)")
plt.ylabel("Displacement $\Delta$r (m)")
plt.xscale('log')
plt.colorbar()
