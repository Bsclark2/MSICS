...
for k in range(1,601):
    filename='E:\Ben\MSICS Frames\\11142023\MSICS_1.7us_'+f'{k:03}'+'.txt'
...
for i in range(1,Sfactor+1):
    displacement.append(x[0,i:]-x[0,:-i])
for i in range(0,Sfactor+1):
    if i%2==1:
        displacement.append(np.sqrt((x[i,:]-x[0,:])**2+(i*pixelsize)**2))
    else:
        displacement.append(i*pixelsize)
Timelag=[]
for i in range(1,Sfactor+1):
    Timelag.append(taup*i*np.ones(imagesize-i))
for i in range(0,Sfactor+1):
    if i%2==1:
        Timelag.append(np.linspace(imagesize*(i+1)-1,imagesize*(i+1)-2*imagesize+1,imagesize)[::-1]*taup)
    else:
...
plt.figure(dpi=500)
plt.scatter(timearray[1381:],disarray[1381:],cmap='inferno')
plt.scatter(timearray[:1380],disarray[:1380],cmap='inferno')
...
#%%
plt.figure(dpi=500)
plt.scatter(timearray[1381:],corarray[1381:],label='$\psi$ shift')
plt.scatter(timearray[:1380],corarray[:1380],label='$\\xi$ shift')
plt.legend()
plt.xlabel("Timelag $\\tau$ (s)")
plt.ylabel("Correlation G($\Delta r$,$\\tau$)")
plt.xscale('log')
...
#%%
plt.figure(dpi=500)
plt.scatter(timearray[1381:],corarray[1381:],c=disarray[1381:],cmap='inferno_r')
plt.plot(np.array(timearray[1381:]),GDL((np.array(disarray[1381:]),np.array(timearray[1381:])),popt[0],popt[1],popt[2]),c='b')
plt.scatter(timearray[:1380],corarray[:1380],c=disarray[:1380],cmap='inferno_r')
plt.plot(np.array(timearray[:1380]),GDL((np.array(disarray[:1380]),np.array(timearray[:1380])),popt[0],popt[1],popt[2]),c='b')
# plt.plot(np.array(timearray[1381:]),GDL((np.array(disarray[1381:]),np.array(timearray[1381:])),popt[0],popt[1],popt[2]),c='black',linestyle='dashed')
# plt.scatter(timearray[:1380],corarray[:1380],c=disarray[:1380],cmap='inferno_r')
# plt.plot(np.array(timearray[:1380]),GDL((np.array(disarray[:1380]),np.array(timearray[:1380])),popt[0],popt[1],popt[2]),c='black',linestyle='dashed')
plt.xlabel("Timelag $\\tau$ (s)")
...
plt.show()
#%%
K3=0.0 #hysteresis constant determined for particular image parameters
d=0.0 #hysteresis constant for associated shift
#%%
"""
Hysteresis Correction
...
displacement1=[]
for i in range(1,Sfactor+1):
    displacement1.append(x[0,i:]-x[0,:-i])
for i in range(0,Sfactor+1):
    if i%2==1:
        displacement1.append(np.sqrt((x[i,:]-x[0,:])**2+(i*pixelsize)**2))
    else:
        displacement1.append(i*pixelsize)
Timelag1=[]
for i in range(1,Sfactor+1):
    Timelag1.append(taup*i*np.ones(imagesize-i))
for i in range(0,Sfactor+1):
    if i%2==1:
        Timelag1.append(np.linspace(imagesize*(i+1)-1,imagesize*(i+1)-2*imagesize+1,imagesize)[::-1]*taup)
    else:
        Timelag1.append(i*imagesize*taup)

CorrelationV1=AF2blist
for i in range(0,Sfactor+1):
    if i%2==1:
        CorrelationV1.append(AF2alist[int((i-1)/2)])
    else:
        CorrelationV1.append(AF2clist[int(i/2)])
timearray1=flat(Timelag1)
disarray1=flat(displacement1)
corarray1=flat(CorrelationV1)
popt1,pcov1=curve_fit(GDL, (disarray1,timearray1),corarray1,p0=[64.0e-12, 2.19193849,0.55],method='lm')
#%%
plt.figure(dpi=500)
plt.scatter(timearray1[1381:],corarray1[1381:],label='$\psi$ shift')
plt.scatter(timearray1[:1380],corarray1[:1380],label='$\\xi$ shift')
plt.xlabel("Timelag $\\tau$ (s)")
plt.ylabel("Correlation G($\Delta r$,$\\tau$)")
plt.xscale('log')
plt.title('G($\Delta r,\\tau$) vs TimeLag $\\tau$')
...
plt.scatter(timearray[1381:],corarray[1381:],c=disarray[1381:],cmap='inferno_r')
plt.scatter(timearray[:1380],corarray[:1380],c=disarray[:1380],cmap='inferno_r')
plt.plot(np.array(timearray1[1381:]),GDL((np.array(disarray1[1381:]),np.array(timearray1[1381:])),popt1[0],popt1[1],popt1[2]),c='black',linestyle='dashed')
plt.plot(np.array(timearray1[:1380]),GDL((np.array(disarray1[:1380]),np.array(timearray1[:1380])),popt1[0],popt1[1],popt1[2]),c='black',linestyle='dashed',label='Raw')
plt.plot(np.array(timearray[1381:]),GDL((np.array(disarray[1381:]),np.array(timearray[1381:])),popt[0],popt[1],popt[2]),c='b')
plt.plot(np.array(timearray[:1380]),GDL((np.array(disarray[:1380]),np.array(timearray[:1380])),popt[0],popt[1],popt[2]),c='b',label='Hysteresis')
plt.xlabel("Timelag $\\tau$ (s)")
plt.ylabel("Correlation G($\\tau$,$\Delta r$)")
plt.xscale('log')
# plt.colorbar()
